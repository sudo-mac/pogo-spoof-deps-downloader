...
while true; do
    ...
    case $pokechoice in
    [1]*)
        echo && echo "Installing PGSharp spoofers package..."
        ...
        ;;
    [2]*)
        echo && echo "Installing Pokemod spoofers package"
        for device in $(adb wait-for-device devices | grep device$ | cut -f1); do
            adb -s "$device" shell su -c 'exit'
...
while true; do
    ...
    case $yn in
    [yY]*)
        echo && echo "But... which one will it be...?"
        commands=(
        ...
        eval ${commands[$index]}
        sleep 1
        echo && echo "Random Bootanimation Downloaded! (:"
        break
        ;;
    [nN]*)
        echo "Skipping Random Bootanimation... ):"
        break
        ;;
    *) echo "Invalid Response!" ;;
    esac
